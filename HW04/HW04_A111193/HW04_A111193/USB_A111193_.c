...
struct usb_mouse {
    ...
    char phys[64];                  // Physical path of the device
    struct usb_device *usbdev;      // Pointer to the USB device
    struct input_dev *dev;          // Pointer to the input device
    struct urb *irq;                // USB request block for interrupt transfer

    signed char *data;              // Pointer to data buffer
    dma_addr_t data_dma;            // DMA address of the data buffer
};
...
MODULE_DEVICE_TABLE (usb, usb_mouse_id_table); // Macro to export the device ID table
...
// Macro to register the USB driver
